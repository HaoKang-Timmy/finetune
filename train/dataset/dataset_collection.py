...
class DatasetCollection():
    ...
    def init(self):
        if(self.type == 'Imagenet'):
            ...
            images = pd.read_csv(os.path.join(self.path, 'images.txt'), sep=' ',
                                 names=['img_id', 'filepath'])
            image_class_labels = pd.read_csv(os.path.join(self.path, 'image_class_labels.txt'),
                                             sep=' ', names=['img_id', 'target'])
            train_test_split = pd.read_csv(os.path.join(self.path, 'train_test_split.txt'),
            ...
            train_dataset = datasets.CIFAR10(
                root=self.path, train=True, transform=self.compose['train'], download=True)
            val_dataset = datasets.CIFAR10(
                root=self.path, train=False, transform=self.compose['val'])
            return train_dataset, val_dataset
